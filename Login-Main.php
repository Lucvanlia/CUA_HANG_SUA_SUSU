...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action'] == 'register') {
    ...
    // Sử dụng password_hash để mã hóa mật khẩu an toàn hơn
    ...
    if ($link) {
        ...
        if ($result->num_rows > 0) {
            ...
            $insertQuery = "INSERT INTO khachhang (Ten_kh, email_kh, sdt_kh, mk_kh, namsinh_kh) VALUES (?, ?, ?, ?, ?)";
            $stmt = $link->prepare($insertQuery);
            $stmt->bind_param("sssss", $name, $email, $phone, $password, $dob);
            if ($stmt->execute()) {
